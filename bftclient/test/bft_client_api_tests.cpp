...
class ClientApiTestFixture : public ::testing::Test {
 public:
  ...
  void PrintAndVerifySignature(const MsgFromClient& msg,
    ...
    const bftEngine::ClientRequestMsgHeader* request_header =
        reinterpret_cast<const bftEngine::ClientRequestMsgHeader*>(request);
    auto request_data = request + sizeof(bftEngine::ClientRequestMsgHeader) + request_header->spanContextSize;
    if (corrupt_request) {
      if (rand() % 9 < 5)
  ...
  void PrintAndFailVerifySignatureBehavior(const MsgFromClient& msg, IReceiver* client_receiver) {
    PrintAndVerifySignature(msg, client_receiver, false, corrupt_request_);
  }
  ...
  void GenerateKeyPair(string& out) {  // ASSERT_ macros cannot be used in non-void functions
    ostringstream cmd;
  ...
  void EncryptPrivKey(SecretData& out,
                      const string& priv_key_file_path) {  // ASSERT_ macros cannot be used in non-void functions
    ...
    out = move(GetSecretData());  // return secret data
  }

  unique_ptr<bftEngine::impl::RSAVerifier> transaction_verifier_;
...
TEST_P(ClientApiTestParametrizedFixture, print_received_messages_and_timeout) {
  ...
  string scenario = get<2>(GetParam());
  if (sign_transaction) {
    ...
    test_config_.transaction_signing_private_key_file_path_ = file_path.str();

    if (is_priv_key_encrypted) {
      // encrypt the private key, it is  needed for the test
      ostringstream encrypted_file_path;
      SecretData sd;

      encrypted_file_path << keypair_path << ENC_PRIV_KEY_NAME;
      test_config_.transaction_signing_private_key_file_path_ = encrypted_file_path.str();
      EncryptPrivKey(sd, file_path.str());
      test_config_.secrets_manager_config_ = sd;
    }

    // initialize the test's RSAVerifier
    string public_key_full_path({keypair_path + PUB_KEY_NAME});
    transaction_verifier_ =
        unique_ptr<bftEngine::impl::RSAVerifier>(new bftEngine::impl::RSAVerifier(public_key_full_path));
  }
  unique_ptr<FakeCommunication> comm;
  if (sign_transaction) {
    if (scenario == "happy_flow")
      comm = make_unique<FakeCommunication>(bind(&ClientApiTestParametrizedFixture::PrintAndVerifySignatureBehavior, this, _1, _2));
    else if (scenario == "corrupt_in_dest") {
      comm = make_unique<FakeCommunication>(bind(&ClientApiTestParametrizedFixture::PrintAndFailVerifySignatureBehavior, this, _1, _2));
