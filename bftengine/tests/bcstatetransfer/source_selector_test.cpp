...
TEST(source_selector_test, on_construction_there_is_no_current_replica) {
  ASSERT_EQ(
      SourceSelector(replicas, kRetransmissionTimeoutMs, kReplicaReplacementTimeoutMs, maxFetchRetransmissions, GL)
          .currentReplica(),
      NO_REPLICA);
}
...
TEST(source_selector_test, change_source_after_too_many_retransmissions) {
  auto source_selector =
      SourceSelector({}, kRetransmissionTimeoutMs, kReplicaReplacementTimeoutMs, maxFetchRetransmissions, GL);
