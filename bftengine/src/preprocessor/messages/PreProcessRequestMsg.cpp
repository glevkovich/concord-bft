...
void PreProcessRequestMsg::validate(const ReplicasInfo& repInfo) const {
  ...
  if (requestSignature) {
    ...
    if (!sigManager->verifySig(
      ...
      // todo - add info
      LOG_ERROR(logger(),
                "Signature verification failed for " << KVLOG(header->clientId, header->reqSeqNum, this->senderId()));
    ...
    LOG_TRACE(logger(), "Signature verified for " << KVLOG(this->senderId(), header->clientId, header->reqSeqNum));
  }
}
