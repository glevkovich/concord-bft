...
BCStateTran::BCStateTran(const Config &config, IAppState *const stateApi, DataStore *ds)
  ...
  srcGetBlocksContextes_.resize(config_.maxNumberOfChunksInBatch);
  for (uint16_t i{0}; i < config_.maxNumberOfChunksInBatch; ++i) {
    srcGetBlocksContextes_[i].block_ctx.buffer.reset(new char[config_.maxBlockSize]);
    srcGetBlocksContextes_[i].index = i;
  }
...
uint16_t BCStateTran::asyncGetBlocksConcurrent(uint64_t nextBlockId,
  ...
  for (uint64_t i{nextBlockId}; (i >= firstRequiredBlock) && (j < startContextIndex + numBlocks); --i, ++j) {
    auto &ctx = srcGetBlocksContextes_[j];
    ...
    if (ctx.future.valid()) {
      ...
      ctx.future.get();
      LOG_DEBUG(getLogger(),
                "Waiting for previous thread to finish job on context " << KVLOG(ctx.block_ctx.blockId, ctx.index));
    }
    ctx.block_ctx.blockId = i;
    ctx.future = as_->startGetBlockAsync(ctx.block_ctx);
  }
...
bool BCStateTran::onMessage(const FetchBlocksMsg *m, uint32_t msgLen, uint16_t replicaId) {
  ...
  auto isFetchersContextValid = srcGetBlocksContextes_[0].future.valid();
  if (!isFetchersContextValid || srcGetBlocksContextes_[0].block_ctx.blockId != nextBlockId) {
    LOG_INFO(getLogger(),
             "Call asyncGetBlocksConcurrent: source blocks prefetch disabled (first batch or retransmission): "
                 << KVLOG(srcGetBlocksContextes_[0].block_ctx.blockId, nextBlockId));
  ...
  do {
    ...
    auto &src_ctx = srcGetBlocksContextes_[ctx_index];
    auto &block_ctx = srcGetBlocksContextes_[ctx_index].block_ctx;
    ConcordAssert(src_ctx.future.valid());
    waitFutureDuration.start();
    src_ctx.future.get();
    if (!block_ctx.exist) {
      LOG_ERROR(getLogger(), "Block not found in storage, aborting batch:" << KVLOG(src_ctx.index, block_ctx.blockId));
      return false;
    }
    ConcordAssertGT(block_ctx.size, 0);
    ConcordAssertNE(block_ctx.buffer, nullptr);
    ConcordAssertEQ(block_ctx.blockId, nextBlockId);
    uint32_t sizeOfNextBlock = block_ctx.size;
    char *buffer = block_ctx.buffer.get();
    LOG_DEBUG(
        getLogger(),
    ...
    // some statistics
    histograms_.src_get_block_size_bytes->record(block_ctx.size);
    histograms_.src_get_block_duration->record(block_ctx.jobDuration.count());
