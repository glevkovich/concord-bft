...
BCStateTran::BCStateTran(const Config &config, IAppState *const stateApi, DataStore *ds)
  ...
  ConcordAssertLT(processCommitsTimeoutMilli_, config_.refreshTimerMs);
...
bool BCStateTran::processCommitResultsAsync(bool lastBlock, bool breakIfNotReady) {
  ...
  while (!ioContexes_.empty()) {
    ...
    LOG_TRACE(getLogger(), "After putBlockAsync:" << KVLOG(ctx->blockId, nextCommittedBlockId_));
  }
  ...
  while (true) {
    ...
    if (newBlockIsValid && isGettingBlocks) {
      ...
      if (!lastBlock) {
        ...
        //////////////////////////////////////////////////////////////////////////
        if (ioPool_.empty()) {
          // We have a block eady in buffer_, but no free context. let's wait for a job to finish.
          processCommitResultsAsync(lastBlock, false);
        }
        auto ctx = ioPool_.alloc();
