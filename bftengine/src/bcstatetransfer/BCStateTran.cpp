...
void BCStateTran::sourceGetBlock(block_fetcher_context *ctx) {
  ...
  ctx->fetchBlockDurationMicrosec = dt.totalDuration(true);
...
bool BCStateTran::onMessage(const FetchBlocksMsg *m, uint32_t msgLen, uint16_t replicaId) {
  ...
  size_t ctx_index = 0;
  char *buffer;
  DurationTracker<std::chrono::microseconds> waitFutureDuration;
  do {
...
void BCStateTran::cycleEndSummary() {
  ...
  // LOG_INFO(GL, "xxx call gettingMissingBlocksDT_.totalDuration");
  ...
  oss << sources_.back();
  // LOG_INFO(GL, "xxx call cycleDT_.totalDuration");
